...
export const makeScheduleZernio = (options: ResolvedOptions): CollectionAfterChangeHook =>
  async ({ doc, req }) => {
    const shouldSchedule = doc.scheduledAt && doc.postStatus === 'draft'
    if (!shouldSchedule) return doc

    try {
      const settings = await req.payload.findGlobal({ slug: 'social-settings', req })
      const zernio = ((settings as unknown as Record<string, unknown>).zernio ?? {}) as Record<
        string,
        string | undefined
      >
      ...
      if (doc.image) {
        const media = await req.payload.findByID({
          collection: 'media',
          id: typeof doc.image === 'object' ? (doc.image as { id: string }).id : doc.image,
          req,
        })
        const serverURL = req.payload.config.serverURL ?? ''
        const rawUrl: string = (media as unknown as Record<string, unknown>)?.url as string ?? ''
      ...
      const platforms = (doc.platforms as string[])
        .map(p => ({ platform: p, accountId: zernio[`${p}AccountId`] }))
        .filter((p): p is { platform: string; accountId: string } => Boolean(p.accountId))

      if (platforms.length === 0) {
        req.payload.logger.warn(
          `SocialPost ${doc.id}: no account IDs configured in SocialSettings for the selected platforms`,
        )
        return doc
      }
      ...
      const zernioData = await zernioRes.json() as Record<string, string>
      // field name may be 'id', 'postId', or '_id' — verify at integration time
      const zernioPostId: string = zernioData.id ?? zernioData.postId ?? zernioData._id ?? ''
      ...
      try {
        ...
        req.payload.logger.error(
          `Could not save failure state for SocialPost ${doc.id}: ${updateErr}`,
        )
      }
